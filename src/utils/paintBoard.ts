...
export class PaintBoard {
  ...
  recordCurrent(type: string) {
    ...
    switch (type) {
      case CANVAS_ELE_TYPE.FREE_LINE:
        ele = new FreeLine(this.currentLineColor, this.currentLineWidth, this.layer.current);
        break;
        ...
        if (ele?.layer && showLayerIds.has(ele.layer)) {
          ...
          switch (ele?.type) {
            case CANVAS_ELE_TYPE.FREE_LINE:
              ...
              break;
            case CANVAS_ELE_TYPE.CLEAN_LINE:
              cleanLineRender(this.context, this.cleanCanvas.bind(this), ele as CleanLine);
              break;
            default:
              break;
          }
  ...
  cache() {
    ...
    const { currentLineColor, currentLineWidth, cleanWidth, originTranslate, layer } = this;
    const state = {
      currentLineColor,
      currentLineWidth,
      cleanWidth,
